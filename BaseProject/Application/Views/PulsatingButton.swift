...
//  BaseProject
//
//  Created by Tam Le on 17/12/2021.
...
open class PulsatingButton: UIControl {
    ...
    @IBInspectable open var repeatCount: Int = 999999999 {
        didSet {
            updateAnimation()
        }
    }
    ...
    open func start() {
        ...
        for layer in pulseLayers {
            ...
            let pulseAnimation = CABasicAnimation().makeAnimation(keyPath: "transform.scale", duration: pulseDuration, from: 0, to: scaleFactor, beginTime: intervalTime * multiplier, timing: CAMediaTimingFunctionName.default.rawValue, repeatCount: Float(repeatCount))
            let opacityAnimation = CABasicAnimation().makeAnimation(keyPath: "opacity", duration: pulseDuration, from: 1, to: 0, beginTime: intervalTime * multiplier, timing: CAMediaTimingFunctionName.default.rawValue, repeatCount: Float(repeatCount))
...
public extension CABasicAnimation {
    func makeAnimation(keyPath: String?, duration: Double, from: CGFloat, to: CGFloat, beginTime: Double, timing: String?, repeatCount: Float) -> CABasicAnimation {
        ...
        animation.timingFunction = CAMediaTimingFunction(name: timing.map { CAMediaTimingFunctionName(rawValue: $0) } ?? CAMediaTimingFunctionName.default)
